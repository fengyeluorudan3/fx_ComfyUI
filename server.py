...
# ── AstralMind 后端 base URL 唯一定义处 ──
# fx_comfyui_manager 等插件统一从这里读取（lib/utils/astral_urls.py），改这里即可全局生效。
# 也可用环境变量 BRAND_AUTH_BASE_URL / ASTRALMIND_AI_BASE_URL 覆盖。
ASTRALMIND_API_BASE_URL = (
    os.environ.get("BRAND_AUTH_BASE_URL")
    or os.environ.get("ASTRALMIND_AI_BASE_URL")
    or "https://api.astralmindai.com"
).rstrip("/")
...
async def request_log_middleware(request: web.Request, handler):
    ...
    async def _check_token(token: str) -> bool:
        ...
        if not token:
            return False
        validate_url = f"{ASTRALMIND_API_BASE_URL}/users/validateUser"
        try:
            params = {"token": token}
...
class PromptServer():
    def __init__(self, loop):
        ...
        middlewares = [request_log_middleware, cache_control, deprecation_warning]
