...
from fx_crawler_core.model.m_douyin import VideoUrlInfo, CreatorUrlInfo
from fx_crawler_core.tools.crawler_util import extract_url_params_to_dict
from fx_crawler_core.tools.js_runtime import prepare_execjs_node, run_js_file_call
from fx_crawler_core._paths import pkg_path

_douyin_sign_obj = None
_DOUYIN_JS = pkg_path("libs/douyin.js")


def _get_douyin_sign_obj():
    """懒加载 douyin.js 签名；GUI 环境强制 Node，失败则走 node 子进程。"""
    global _douyin_sign_obj
    if _douyin_sign_obj is not None:
        return _douyin_sign_obj
    prepare_execjs_node()
    try:
        runtime_name = execjs.get().name
        if "Node" not in runtime_name:
            raise execjs.RuntimeUnavailableError(f"need Node.js runtime, got {runtime_name}")
        _douyin_sign_obj = execjs.compile(open(_DOUYIN_JS, encoding="utf-8-sig").read())
    except Exception as exc:
        # 标记 False：后续统一走 node 子进程（避免 Nashorn 等 ES5 引擎解析 ES6 报错）
        _douyin_sign_obj = False  # type: ignore[assignment]
        import logging
        logging.getLogger(__name__).warning(
            "[douyin.help] execjs unavailable (%s), fallback to node subprocess", exc
        )
    return _douyin_sign_obj
...
def get_a_bogus_from_js(url: str, params: str, user_agent: str):
    ...
    obj = _get_douyin_sign_obj()
    if obj is False:
        return run_js_file_call(_DOUYIN_JS, sign_js_name, params, user_agent)
    return obj.call(sign_js_name, params, user_agent)
